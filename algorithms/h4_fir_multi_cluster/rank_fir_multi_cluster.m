function [rankx, FeaIndex,FeaNumCandi,Score] = rank_fir_multi_cluster(fea,FeaNumCandi,options)
...
switch lower(options.Method)
    case {lower('LASSO_LARs')}
        ...
        Score=cell(1,length(FeaNumCandi));
        for i = 1:length(FeaNumCandi)
            eigvector = eigvectorAll{i};
            eigvector = max(abs(eigvector),[],2);
            oriscore=eigvector;  %%% 修改的部分
